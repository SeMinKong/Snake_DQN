...
**Deep Q-Network(DQN)**으로 Snake 게임을 학습하는 프로젝트입니다. 픽셀을 입력으로 받는 CNN 모델과 직접 추출한 특징 벡터를 입력으로 받는 선형 모델을 각각 구현해 비교했습니다.
...
- **Double DQN**: 행동 선택은 온라인 네트워크, 가치 평가는 타깃 네트워크가 맡도록 분리했습니다.
- **학습 설정**: Huber Loss(SmoothL1), 그래디언트 클리핑, 지수 감쇠 Epsilon을 적용했습니다.
- **균형 잡힌 보상 설계**: 사망/먹이/이동 보상을 재조정하여 조기 사망 유인 등 왜곡된 인센티브를 제거했습니다.
- **Experience Replay & Target Network**: 두 모델 모두 리플레이 버퍼와 타깃 네트워크를 사용합니다.
...
DQN 하이퍼파라미터, 보상 함수, 상태 표현은 [상세 매뉴얼](./DETAILS.md)에 정리했습니다.
